...
const bash_completions =
    ...
    \\
    \\  case "$prev" in
    \\    attach|run|kill|history|rename)
...
const zsh_completions =
    ...
    \\    args)
    \\      case $words[2] in
    \\        attach|a|kill|k|run|r|history|hi|rename|rn)
    \\          _zmx_sessions
    \\          ;;
    \\        completions|c)
...
const fish_completions =
    ...
    \\
    \\complete -c zmx -n "__fish_seen_subcommand_from attach run kill history rename" -a '(zmx list --short 2>/dev/null)' -d 'Session name'
    \\
